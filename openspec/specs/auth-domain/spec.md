...
This is a cross-cutting capability. It owns the `User` entity, the
...
(`openspec/specs/api-bootstrap/spec.md`); the schema for the `users` and
`refresh_tokens` tables is in `openspec/specs/database-schema.dbml`.
